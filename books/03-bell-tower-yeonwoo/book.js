...
(() => {
  ...
  function render() {
    ...
    // 로컬 스토리지에 진행 저장
    try { localStorage.setItem('bell-tower-progress', String(currentIdx)); } catch (e) {}
  }
  ...
  function startBGM() {
    ...
    try {
      ...
      master.gain.exponentialRampToValueAtTime(0.06, ctx.currentTime + 3); // 살며시 페이드인
      master.connect(ctx.destination);
      // C - Am - F - G  (포근한 자장가풍 진행)
      const chords = [
  ...
  async function init() {
    ...
    let saved = null;
    try {
      const v = localStorage.getItem('bell-tower-progress');
