...
(() => {
  ...
  // ── 잔잔한 BGM (외부 파일 없이 Web Audio로 부드러운 화음 패드 무한 재생) ──
  function startBGM() {
    if (bgm) return;
    try {
      ...
      master.gain.exponentialRampToValueAtTime(0.075, ctx.currentTime + 2.5); // 살며시 페이드인
      master.connect(ctx.destination);
      // Em - C - G - D  (씩씩하고 모험적인 여정 풍 진행)
      const chords = [
      ...
      const melody = [659.25, 523.25, 587.33, 440.00]; // 반짝이는 모험 테마(한 옥타브 위)
      ...
      const playChord = () => {
        ...
        const dur = 2.4;
        // 1) 낮은 베이스 — 둥둥, 모험의 추진력
        const root = chords[state.idx][0] / 2;
        const bo = ctx.createOscillator();
        const bg = ctx.createGain();
        bo.type = 'triangle';
        bo.frequency.value = root;
        bg.gain.setValueAtTime(0, now);
        bg.gain.linearRampToValueAtTime(0.42, now + 0.08);
        bg.gain.linearRampToValueAtTime(0, now + dur * 0.9);
        bo.connect(bg); bg.connect(master);
        bo.start(now); bo.stop(now + dur);
        // 2) 화음 패드
        ...
        mo.type = 'sine';
        mo.frequency.value = melody[state.idx];
        mg.gain.setValueAtTime(0, now + 0.15);
        mg.gain.linearRampToValueAtTime(0.16, now + 0.35);
        mg.gain.linearRampToValueAtTime(0, now + dur * 0.8);
        mo.connect(mg); mg.connect(master);
        mo.start(now + 0.15); mo.stop(now + dur);
        state.idx = (state.idx + 1) % chords.length;
        state.timer = setTimeout(playChord, dur * 1000 * 0.92);
      };
      bgm = {
        stop() {
          state.stopped = true;
