...
(() => {
  ...
  function render() {
    ...
    // 로컬 스토리지에 진행 저장
    try { localStorage.setItem('yeonwoo-gallery-progress', String(currentIdx)); } catch (e) {}
  }
  ...
  async function init() {
    ...
    let saved = null;
    try {
      const v = localStorage.getItem('yeonwoo-gallery-progress');
